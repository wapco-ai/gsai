...
def detect_optimal_batch_size():
    ...
    # Try using nvidia-smi for a more reliable memory query
    try:
        ...
        mem_str = result.stdout.strip().splitlines()[0]
        total_memory = float(mem_str) / 1024  # Convert MB to GB
    except Exception:
        pass

    # Fallback to TensorFlow method if nvidia-smi failed
    if total_memory is None:
        try:
            ...
            logging.warning(f"Failed to detect GPU memory via TensorFlow: {e}")
    ...
    if total_memory is not None:
        if total_memory >= 8:
            ...
            batch_size = 4
        logging.info(
            f"🎮 GPU Memory: {total_memory:.1f}GB, Optimal batch size: {batch_size}"
        )
    else:
...
def save_results_batch_optimized(
    results,
    band_output_folder,
    target_band=0,
    create_blended=False,
    blended_output_folder=None,
):
    ...
    for img_path, predicted_mask in results.items():
        try:
            ...
            band_path = save_band_modified_image_fast(
                img_path, predicted_mask, band_output_folder, target_band
            )
            if band_path:
            ...
            if create_blended and blended_output_folder:
                blended_path = create_colored_mask_and_blend_fast(
                    img_path, predicted_mask, blended_output_folder
                )
                if blended_path:
...
def save_band_modified_image_fast(
    image_path, predicted_mask, band_output_folder, target_band=0
):
    ...
    try:
        ...
        mask_resized = Image.fromarray(predicted_mask.astype(np.uint8)).resize(
        ...
        save_path = os.path.join(
            band_output_folder, f"{base_filename}_band_{target_band}.png"
        )
...
def create_colored_mask_and_blend_fast(image_path, raw_mask, blended_output_folder):
    ...
    try:
        ...
        # Resize mask
        mask_resized = Image.fromarray(raw_mask.astype(np.uint8)).resize(
            original_size, Image.NEAREST
        )
...
def classify_images_ultra_fast(
    image_folder,
    band_output_folder,
    target_band=0,
    create_blended=False,
    ...
    logging.info(
        f"📊 Processing {len(image_paths)} images with batch size {batch_size}"
    )
...
def process_images_band_only(
    image_folder="test_images", output_folder="band_output", target_band=0
):
...
def classify_images_in_folder_with_band(
    image_folder,
    band_output_folder,
    target_band=0,
    create_blended=False,
...
def save_segmentation_in_band(
    predicted_mask, original_image, original_size, output_path, target_band=0
):
...
def create_colored_mask_and_blend(original_image_path, raw_mask, blended_output_folder):
    """Legacy function - kept for compatibility"""
    return create_colored_mask_and_blend_fast(
        original_image_path, raw_mask, blended_output_folder
    )


def save_band_modified_image(
    original_image_path, raw_mask, band_output_folder, target_band=0
):
    """Legacy function - kept for compatibility"""
    return save_band_modified_image_fast(
        original_image_path, raw_mask, band_output_folder, target_band
    )
...
if __name__ == "__main__":
    ...
    process_images_band_only(
        image_folder="test_images", output_folder="turbo_output", target_band=0
    )
