...
Select the encoding of each point‑cloud format with `--ply-mode` and
`--pcd-mode`. Allowed values are `binary`, `ascii`, or `both` (default
`binary`).  Preview files can be generated by specifying the percentage of
points to keep, e.g. `--ply-preview-pct 25`:

```bash
metashape -r metashape_script.py --image_full_pipeline \
    --image_dir path/to/images --output_dir outputs/run1 \
    --export_ply --ply-mode ascii --ply-preview-pct 25
```

The upload forms expose these options via dropdowns and percentage fields for
PLY and PCD.
